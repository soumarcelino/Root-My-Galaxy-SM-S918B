...
/* source: raw disasm of FUN_00103e18 this session (docs/kernel-reference/
 ...
 *      dance -- see docs/kernel-reference/README.md for why it and the
 ...
 * futex_trigger.c/docs/kernel-reference/README.md for full derivation),
 ...
 * source (docs/kernel-reference/locking/rtmutex_api.c:436,
 ...
 * derived from real kernel source this session (docs/kernel-reference/
 ...
 * (docs/kernel-reference/futex/core.c, extracted from
 ...
 * via `rt_mutex_wait_proxy_lock()` (docs/kernel-reference/locking/
...
/* Valid only from v14's waiter callback. Reuses the live waiter/consumer,
 * rewrites its FPSIMD waiter, publishes `phase->sequence`, and waits for the
 * matching sched_setattr completion. */
int futex_v14_deliver_persistent_phase(const struct oss_futex_phase *phase,
                                       uint64_t ghost_task);

/* Item 2: CPU-quiet window. begin() is fired by the trigger just before
 * sched_setattr; the post-trigger callback calls end() right after the fops
 * restore to re-online the siblings for the root stage. Both are idempotent
 * and best-effort (offlining a CPU needs privilege). */
