...
```text
targets/afzh3/kernelsu-next/out/kernelsu-next-afzh3-v3.4.0/
...
anteriormente para `targets/afzh3/kernelsu-next/`. Sua identidade local é:
...
| Patch Samsung | `targets/afzh3/kernelsu-next/patches/KernelSU-Next-v3.4.0-samsung-afzh3-kdp-rkp-defex.patch` |
...
`targets/afzh3/kernelsu-next/patches/`; não é necessário editar manualmente os 15 arquivos
...
export KERNEL_OUT="$RMG/targets/afzh3/kernelsu-next/out/afzh3-kernel"
```

Execute a preparação:

```sh
JOBS=4 "$RMG/targets/afzh3/kernelsu-next/prepare-afzh3-kernel-v3.4.0.sh"
```
...
3. Extrai a configuração da imagem de boot para
   `targets/afzh3/kernelsu-next/out/afzh3-kernel/.config`.
...
```text
targets/afzh3/kernelsu-next/out/afzh3-kernel/
```
...
```sh
JOBS=8 "$RMG/targets/afzh3/kernelsu-next/build-afzh3-v3.4.0.sh"
```
...
embute o `.ko` **oficial** copiado para `targets/afzh3/kernelsu-next/` e gera
...
```sh
OUT="$RMG/targets/afzh3/kernelsu-next/out/kernelsu-next-afzh3-v3.4.0"
cd "$OUT"
...
`/data/local/tmp/.ksud-stage`. O helper de `targets/afzh3/helper/su_daemon.c` espera o primeiro
...
JOBS=4 "$RMG/targets/afzh3/kernelsu-next/prepare-afzh3-kernel-v3.4.0.sh"
JOBS=8 "$RMG/targets/afzh3/kernelsu-next/build-afzh3-v3.4.0.sh"
cd "$RMG/targets/afzh3/kernelsu-next/out/kernelsu-next-afzh3-v3.4.0"
sha256sum -c SHA256SUMS
modinfo -F vermagic android13-5.15_kernelsu.ko
```
...
```text
/home/matias/Projects/Root-My-Galaxy-SM-S918B/targets/afzh3/kernelsu-next/out/kernelsu-next-afzh3-v3.4.0/ksud-next-v3.4.0
/home/matias/Projects/Root-My-Galaxy-SM-S918B/targets/afzh3/kernelsu-next/out/kernelsu-next-afzh3-v3.4.0/android13-5.15_kernelsu.ko
```
