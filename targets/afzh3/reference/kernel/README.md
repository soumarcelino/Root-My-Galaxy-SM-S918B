...
- `rtmutex_common.h` -- `struct rt_mutex_waiter` layout (confirms this
  project's `FAKE_WAITER_*` offsets in `targets/afzh3/reference/kernel/legacy-target/target.h` are
...
two functions actually ported into `targets/afzh3/open-payload-engine/` so far
...
the live `file_operations` table. `targets/afzh3/open-payload-engine/src/fops_install.c`
...
`targets/afzh3/open-payload-engine/src/aar_aaw.c`. **Never yet proven to reach real
...
long-term. It is not -- see `targets/afzh3/open-payload-engine/STATUS.md`'s "CORRECTION
...
`targets/afzh3/open-payload-engine/src/root_umh.c` remains a verified-equivalent-purpose
substitute (confirmed via exact `target.h` offset matches inside
`FUN_00108fa4`'s decompile) rather than a literal port of this
subsystem.

`targets/afzh3/open-payload-engine/src/root_umh.c` currently substitutes a
...
`test_root_v3` fired once on-device (see `targets/afzh3/open-payload-engine/STATUS.md`
...
This local BTF file (`targets/afzh3/reference/kernel/btf/vmlinux-SAFZH3-5.15.189.btf`
...
**Practical port decision made from this evidence**: `targets/afzh3/open-payload-engine`'s
