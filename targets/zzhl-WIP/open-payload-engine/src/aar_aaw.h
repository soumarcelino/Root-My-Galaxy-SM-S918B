...
/* Same verification with an exact expected fake-fops address and a sticky
 * landing result. The caller initializes landed to zero; this function
 * release-stores 1 after any full eight-byte read at the kernel target (which
 * native size-zero ashmem cannot produce), even if the pointer value or a
 * later R/W round trip is wrong. */
...
/* Split of oss_verify_kernel_access_ex into its two independent checks so the
 * post-trigger flow can prove landing, restore ashmem_misc.fops, and only then
 * run the R/W round trip. oss_verify_fops_landed does step 1 (single readback,
 * no writes; sets *landed like the _ex variant). oss_verify_magic_rw does step
 * 2 (magic write/read/compare) and is safe to call after the fops restore
 * because the open fd keeps its fake f_op. */
