...
#define SIGUSR1_PAYLOAD_SIZE 0x200U
 ...
 * targets/afzh3/reference/kernel/README.md ("Major reframing: the real trigger
...
/* Serialize the generic closed-route waiter image.  The local handler copies
 * payload byte zero to fpsimd_context.vregs (record +0x10), while the closed
 * handler copies its source byte zero to record +0x18.  The closed source's
 * waiter starts at +0x10, so the equivalent waiter base in this payload is
 * +0x18 and its pi_tree node begins at payload +0x30.
 * This pure helper exists so the byte layout can be tested without signals. */
...
/* Install the serialized waiter image as the next handler payload. */
 ...
 * if the handler doesn't run in time). sigusr1_install_handler() and one
 * payload builder must both have been called first. Returns 1
